...
A minimal substrate hook. Every pipeline run should emit at least one entry to a
local NDJSON ledger; if ``AUDIT_HOST`` is set, the entry is also POSTed to the
substrate audit-API at ``http://${AUDIT_HOST}/events``.

The chain is intentionally simple: each entry's ``prev_hash`` field is the
SHA-256 of the previous entry's canonical JSON encoding. Tampering is detectable
...
Schema::

    {
        ...
        "action": "ablation_start",
        "actor": "mocnv@host",
        "job_id": "demo-2026-06-04-17",
...
UTC = timezone.utc  # noqa: UP017 — keep importable on Python 3.10 sandboxes too
...
def emit(
    ...
    *,
    ...
    if should_post and audit_host:
        # Substrate down is not pipeline-fatal; the local NDJSON is the source of
        # truth and the remote post is best-effort.
        with contextlib.suppress(requests.RequestException):
            requests.post(f"http://{audit_host}/events", json=entry, timeout=2.0)
