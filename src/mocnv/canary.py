...
Every demo repo exposes a canary entry point that the ``lab_semantic_check.py``
runner can probe daily. The canary must:

1. Complete in under 30 seconds on a single workstation.
2. Produce deterministic output given a fixed fixture input.
3. Return exit code 0 on success, non-zero on any deviation.

The default fixture exercises the audit emit and tracking modules without any
external services. Override the fixture path via the ``MOCNV_CANARY_FIXTURE``
...
from mocnv import audit, tracking
...
def check() -> dict[str, Any]:
    ...
    fixture_path = Path(os.environ.get("MOCNV_CANARY_FIXTURE", str(DEFAULT_FIXTURE)))
    ...
    audit.emit(action="canary_end", job_id=job_id, fields={"ok": True})

    return {"ok": True, "job_id": job_id, "fixture": fixture}
