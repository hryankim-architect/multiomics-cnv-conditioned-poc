...
If ``MLFLOW_TRACKING_URI`` is not set, the wrapper becomes a no-op so the demo
still runs on a fresh checkout without an MLflow server. When the substrate is
present, runs are tracked against the configured URI.
"""
